...
class GaodePOI {
    ...
    _formatPOIData(arr) {
        ...
        for (var i = 0; i < arr.length; i++) {
            var item = arr[i];
            var coor = [];
            if (item.location && item.location.length>0) coor = okay3d.pointconvert.gcj2wgs(item.location.split(",")); //坐标
            arrNew.push({
                id: item.id,
                name: item.name, //名称
                x: coor[0],
    ...
    query(opts) {
        ...
        if (entity) {
            if (entity.rectangle) {
                var coors = okay3d.draw.attr.rectangle.getCoordinates(entity);
                opts.points = coors;
                that.queryPolygon(opts);
            }
            if (entity.polygon) {
                var coors = okay3d.draw.attr.polygon.getCoordinates(entity);
                opts.points = coors;
                that.queryPolygon(opts);
            }
            if (entity.ellipse) {
                var coor = okay3d.draw.attr.circle.getCoordinates(entity)[0];
    ...
    queryCircle(opt) {
        ...
        //坐标构造 location
        var cartg = okay3d.pointconvert.wgs2gcj([opt.x, opt.y]);
    ...
    queryPolygon(opt) {
        ...
        if (coors.length == 2) { //矩形
            var cartg1 = okay3d.pointconvert.wgs2gcj(coors[0]);
            var cartg2 = okay3d.pointconvert.wgs2gcj(coors[1]);
            ...
            for (var i = 0; i < newCoors.length; i++) {
                var cartg = okay3d.pointconvert.wgs2gcj(newCoors[i]);
