...
class QueryArcServer {
    constructor(options) {
        ...
        this.queryWFS = okay3d.L.esri.query({
            url: options.url
        });
    ...
    query(opt) {
        ...
        if (opt.fwEntity) {
            ...
            if (drawEntity.polygon) {
                this.queryWFS.intersects(okay3d.draw.attr.polygon.toGeoJSON(drawEntity));
            }
            else if (drawEntity.rectangle) {
                var coor = okay3d.draw.attr.rectangle.getOutlineCoordinates(drawEntity);
                this.queryWFS.intersects(okay3d.L.latLngBounds(okay3d.L.latLng(coor[0][1], coor[0][0]),
                    okay3d.L.latLng(coor[2][1], coor[2][0])));
            }
                ...
                var buffere = turf.buffer(okay3d.draw.attr.ellipse.toGeoJSON(drawEntity), radius, { units: 'meters' });
        ...
        if (featureCollection == undefined
            ...
            dataSource.then(function (dataSource) {
                ...
                for (var i = 0, len = entities.length; i < len; i++) {
                    ...
                    //属性
                    var attr = okay3d.util.getAttrVal(entity.properties);
                    ...
                    if (that.options.popup) {
                        entity.popup = {
                            html: function (entity) {
                                ...
                                return okay3d.util.getPopup(that.options.popup, attr, that.name);
                            },
                            anchor: [0, -15],
                        };
                    }
                    ...
                    if (attrLbl) {
                        ...
                        attrLbl = okay3d.draw.util.getDefStyle('label', attrLbl)//默认值

                        //entity属性
                        entity.label = okay3d.draw.attr.label.style2Entity(attrLbl);
                    }
                }
